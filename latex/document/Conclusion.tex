...
parameters which tremendously reduces the required number of \ac{FLOPS}\index{\ac{FLOPS}} and in turn training time to reach satisfactory 
...
ImageNet\cite{Deng2009}\index{ImageNet} has around $14$ millions images and a relatively new model 
like GoogleNet\cite{Szegedy2014}\index{GoogleNet}, depending on the instance that you use, has millions of parameters 
...
\resizebox{1.0\textwidth}{!}{
  \subfloat[Proposed Winning Scenario for \ac{UWF}\index{\ac{UWF}} Using \optuna\cite{Akiba2019}\index{\optuna}: Different Scalars Multiplied by a Single Matrix$(\tau_k\boldsymbol{M})$, $\mathrm{lr}=8.798\times10^{-3}, \,\mathrm{L}=160$]{\input{./tikz/wf/optuna.tex}}\\  
% }
% \resizebox{0.67\textwidth}{!}{
  \subfloat[Proposed Winning Scenario for \ac{URWF}\index{\ac{URWF}} Using \optuna\cite{Akiba2019}\index{\optuna}: Different Scalars Multiplied by a Single Matrix$(\tau_k\boldsymbol{M})$, $\mathrm{lr}=7.622\times10^{-3}, \,\mathrm{L}=30$]{\input{./tikz/rwf/optuna.tex}}\\  
}
  \caption{Proposed Winning Scenario for \ac{UWF}\index{\ac{UWF}} and \ac{URWF}\index{\ac{URWF}} after \HO Using \optuna\cite{Akiba2019}\index{\optuna}}
