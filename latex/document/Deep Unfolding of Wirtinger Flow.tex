...
\begin{equation}\label{eq:l_modulation_effect}
	...
	\end{split}
  \end{equation}
\cite{Candes2013} proved that if the number of modulations/masks $L$ is high enough, then the recovery of the ray is possible using \ac{PR} methods. 
...
There are however some restrictions regarding the $d[t]$ in \cref{eq:modulation_effect} and \cref{eq:l_modulation_effect}. \cite{Candes2013} proposed:
\begin{equation}
...
As it is stated by \cite{Candes2013} different \ac{CDP} might be better suited for different applications. We provide a double checking of the \emph{Octanary Modulation} 
...
\afterpage{%
\clearpage % Start a new page
\begin{algorithm}\label{pseudocode:imrwf}
