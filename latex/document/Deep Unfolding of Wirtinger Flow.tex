...
,\boldsymbol{W}^{N+1} \in \mathbb{R}^{m \times k_N},\boldsymbol{W}^{l+1} \in \mathbb{R}^{k_l \times k_{l+1}}$ where $m,n,l,k_l \in \mathbb{N}$ \todo{You need $l$ from $1$ to $n$ for the $k_l$ and $l$ from $1$ to $n-1$ for the $\boldsymbol{W}^{l+1}$} and $\varphi$ is a
function from $\mathbb{R}$ to $\mathbb{R}$ with certain properties\todo{Most import is that it's nonlinear. Otherwise, the whole network would be linear. You mention this further down}. 
...
\emph{weight matrices}, \emph{biases}, and \emph{activation function}. \ac{MLP}s were designed to approximate outputs from inputs without knowing the the actual mapping and with observations and measurements\todo{observations and measurements? You need input-output pairs (which you hint at the beginning of the line.). Or what do you }\todo{You need to mention/explain the concept of training.}.\todo{You need to explain somewhere what layers are.} 
...
periods known as the \emph{\ai winters} \todo{Are the AI winters relevant for what you want to say? You only mention very few things, so I would make sure that you concentrate on things important for your work or the point you want to make} that meant the substantial reduction in \ml/\dl/\ai funding and 
...
\begin{itemize}
  \item can extract extremely complicated mappings or extremely subtle features\todo{I would also put a space before an opening bracket}(depending on the desired wording),
  ...
  \item currently can beat human level performance in lots of areas,\todo{Better than humans in lots of areas? Then, please name a number of those areas.}
\end{itemize}
...
\begin{itemize}
  ...
  \item requires tremendous raw computational power at the level of \todo{I don't think this works gramatically.} \cite{Meuer} and storage which in turn would result in large electricity bills
  \footnote{in $2022$ Matthias M\"{u}ller, head of the \ac{HPC} center at RWTH Aachen University, explained his rule of thumb as  
  one Megawatt costing them one million euros per year.}\todo{You mean in a course? A random quote is not really a good reference...} and expensive maintenance costs,
...
Due to the pros and cons tied to \dl what is getting a bit of traction at least in \dsip and \cv is \du or \au. 
\du or \au \todo{Don't use these two terms together all the time. Introduce both, but then stick to one of them.} is the process of \emph{unfolding}/\emph{unrolling} an iterative algorithm finite times and training certain parameters in 
the hopes of improving some metric by putting the \emph{unrolled} algorithm in a model\todo{What does putting the unrolled algorithm in a model mean? Is this meant to say that a fixed number of iterations of a given algorithm are re-intereted as a neural network? This is the point and must be made clear.}. It is best to look at the process in \cref{fig:deep_unfolding_unrolling} \todo{You need to properly explain this. This is a core concept of the thesis and needs to be made crystal clear. Iterations are converted to layers, linearities and nonlinearities need to be identified, only certain nonlinearities allow for the necessary reinterpretation.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\du/\au tries to bring the best of the both worlds(analytical approach and data driven approach) in one and reconcile the two\todo{This needs to be explained more. By unrolling you get very specific network architectures and you usually have much fewer parameters than typically in deep learning.}. A word of 
...
a closer look will result in a couple of subtleties between the two\todo{I don't see how these things don't make it a RNN. Even if you reduce the number of parameters or have easy initialization, it is still an RNN, albeit a quite specific one. But is out structure actually recurrent? The structure you show in the unfolding figure looks uni-directional, i.e. like a feed forward network.}. Since in the \du/\au you significantly reduce the number of 
...
\emph{unfolded}/\emph{unrolled}. We will explain this in more details when we are describing the training process of our \ml/\dl model in our study.}\todo{This is essential, don't hide it in a footnote.} which for some time were the center of attention in \ml/\dl community \cite{Glorot2010}\cite{He2015a}. A 
nice review on the recent application of \dl\todo{It this a typo? Do you mean deep unfolding?}/\au in \dsip is done by \cite{Monga2019}.
...
\todo{This is apparently just a fragment that it supposed to be extended. Note that we don't know yet (unless the introduction says this) that we need complex values in the network.}
...
\todo{We are in the middle of the deep learning section. You shouldn't jump to Wirting Flow here. Introducing this needs its own section. Above you introduced a tool that we want to use, now you start introducing a problem that we want to look at (phase retrieval) direction combined with a tool that is ofen used to solve it (Wirtinger flow). First, you should cleanly introduce the problem (peaking at what comes, you seem to (partially) do this in 3.3) and then describe how it is usually solved and then how deep funfolding comes in.}
...
determining the phase(up to a global phase) of an image simply because it contains information which is of interest depending on the context\todo{To understand what retrieving the phase means, you first need to have an idea from which data you start.}. 
...
natural images. The reason why the phase gets lost is out of the scope of the current work \todo{You at least need to describe our forward model, i.e., explain how we assume data to be obtained from the image we want to know. Then it's at least mathematically obvious why the phase is lost.} but 
