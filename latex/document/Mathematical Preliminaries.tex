...
\todo{The}\ac{FT} and its discrete counterparts \todo{counterpart} will play an important role in one of our examples and here we give a fast refresher on the basic properties of \ac{DFT} and \ac{IDFT}.

\begin{Def}\label{def:1ddft}
    \emph{$1$-$d$ \ac{DFT}}\\
    The \emph{$1$-$d$ \ac{DFT}} \todo{I personally prefer 1D, but that is a matter of taste. In any case, I would not use an italic d. This is no variable where you insert something.} of the $1$-$d$ array $\boldsymbol{X} \in \mathbb{C}^{N}$ is denoted by 
